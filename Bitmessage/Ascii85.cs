...
public class Ascii85
{
	...
	public byte[] Decode(string s)
	{
		if (EnforceMarks)
		{
            //checks if prefix and suffix are present and in the correct order
            if (!s.Contains(PrefixMark) || !s.Substring(s.IndexOf(PrefixMark)).Contains(SuffixMark))
            {
        ...
        //Extract Ascii85 string
        if (!s.StartsWith(PrefixMark))
        {
            s = s.Substring(s.IndexOf(PrefixMark));
        }
        if (!s.EndsWith(SuffixMark))
        {
            s = s.Substring(0, s.IndexOf(SuffixMark) + SuffixMark.Length);
        }
